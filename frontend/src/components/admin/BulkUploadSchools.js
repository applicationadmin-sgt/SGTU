...
const BulkUploadSchools = ({ onUpload }) => {
      ...
      Papa.parse(e.target.files[0], {
        ...
        complete: (results) => {
          ...
          rows.forEach((row, idx) => {
            ...
            // Validate code format (should be uppercase, 2-5 characters)
            if (normalizedRow.code && normalizedRow.code.trim()) {
              const code = normalizedRow.code.trim();
              if (!/^[A-Z]{2,5}$/.test(code)) {
                errors.push({ row: idx + 2, message: 'Code should be 2-5 uppercase letters (e.g., SOE, SOM)' });
              }
            }
          });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setSuccess(result?.message || 'Schools created successfully');
      setFile(null);
      setPreview([]);
      setCsvErrors([]);
      setActiveStep(0);
    } catch (err) {
      setError(err.response?.data?.message || 'Bulk school creation failed');
  ...
  return (
    ...
    >
      ...
      <CardContent sx={{ p: 3 }}>
        ...
        {file && (
          ...
          >
            <Typography variant="body2" sx={{ fontWeight: 500 }}>
              Selected file: <strong style={{ color: '#673ab7' }}>{file.name}</strong> ({Math.round(file.size / 1024)} KB)
            </Typography>
          </Box>
        )}
        ...
        {isUploading && (
          <Box sx={{ mt: 3 }}>
            <LinearProgress sx={{ borderRadius: 1, height: 6, backgroundColor: '#e1bee7', '& .MuiLinearProgress-bar': { backgroundColor: '#673ab7' } }} />
            <Typography variant="body2" sx={{ mt: 1.5, textAlign: 'center', fontWeight: 500, color: '#673ab7' }}>
              Creating schools...
            </Typography>
          </Box>
        )}
      ...
      <Collapse in={expanded}>
        ...
        <CardContent sx={{ backgroundColor: '#fafafa', p: 3 }}>
          {preview.length > 0 && (
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600, color: '#673ab7' }}>
              ...
              >
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow>
                      <TableCell sx={{ fontWeight: 700, backgroundColor: '#673ab7', color: 'white' }}>Row</TableCell>
                      {Object.keys(preview[0]).map(h => (
                        <TableCell key={h} sx={{ fontWeight: 700, backgroundColor: '#673ab7', color: 'white', textTransform: 'capitalize' }}>{h}</TableCell>
                      ))}
                    </TableRow>
                  </TableHead>
                  <TableBody>
          ...
          <Box>
            ...
            >
              <Table size="small">
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ fontWeight: 700, backgroundColor: '#673ab7', color: 'white' }}>Field</TableCell>
                    <TableCell sx={{ fontWeight: 700, backgroundColor: '#673ab7', color: 'white' }}>Description</TableCell>
                    <TableCell sx={{ fontWeight: 700, backgroundColor: '#673ab7', color: 'white' }}>Example</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  <TableRow sx={{ '&:nth-of-type(odd)': { backgroundColor: '#f5f5f5' } }}>
                    ...
                    <TableCell sx={{ fontFamily: 'monospace', color: '#673ab7' }}>School of Engineering</TableCell>
                  </TableRow>
                  <TableRow sx={{ '&:nth-of-type(odd)': { backgroundColor: '#f5f5f5' } }}>
                    <TableCell sx={{ fontWeight: 600 }}>code</TableCell>
                    <TableCell>School code - 2-5 uppercase letters (required)</TableCell>
                    <TableCell sx={{ fontFamily: 'monospace', color: '#673ab7' }}>SOE, SOM, SOA</TableCell>
                  </TableRow>
                  <TableRow sx={{ '&:nth-of-type(odd)': { backgroundColor: '#f5f5f5' } }}>
                    <TableCell sx={{ fontWeight: 600 }}>description</TableCell>
                    <TableCell>School description (required)</TableCell>
                    <TableCell sx={{ fontFamily: 'monospace', color: '#673ab7' }}>School of Engineering and Technology</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </TableContainer>
            ...
            >
              <Typography variant="body2" sx={{ lineHeight: 1.8 }}>
                <strong>Note:</strong> School codes must be unique. Use 2-5 uppercase letters (e.g., SOE, SOM, SOL). 
                After creating schools, you can add departments and assign deans.
              </Typography>
            </Alert>
          </Box>
        </CardContent>
