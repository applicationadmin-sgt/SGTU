...
const TeacherLiveClassDashboard = ({ token, user }) => {
  ...
  link.href = `${process.env.REACT_APP_API_URL || 'https://10.20.49.165:5000'}${classItem.recordingUrl}`;
        ...
        <DialogContent>
          {selectedRecording?.recordingUrl && (
            <Box sx={{ mb: 2 }}>
              ...
              >
                <source 
                  src={`${process.env.REACT_APP_API_URL || 'https://10.20.49.165:5000'}${selectedRecording.recordingUrl}`} 
                  type="video/webm"
                />
