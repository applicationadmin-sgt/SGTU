...
const TeacherLiveClassDashboard = ({ token, user }) => {
  ...
  link.href = `${process.env.REACT_APP_API_URL || 'https://192.168.7.20:3000'}${classItem.recordingUrl}`;
        ...
        <DialogContent>
          {selectedRecording?.recordingUrl && (
            <Box sx={{ mb: 2 }}>
              ...
              >
                <source 
                  src={`${process.env.REACT_APP_API_URL || 'https://192.168.7.20:3000'}${selectedRecording.recordingUrl}`} 
                  type="video/webm"
                />
