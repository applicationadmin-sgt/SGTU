...
const LiveClassRoom = ({ token, user }) => {
  ...
  const initializeClass = async () => {
    try {
      ...
      const isTeacherByRole = user.roles ? user.roles.includes('teacher') : user.role === 'teacher';
      ...
      if (!isTeacher && (user.roles ? user.roles.includes('student') : user.role === 'student')) {
        // Check if student is in the section (this should be handled by the API)
      }
      
      // Initialize socket connection
      socket.current = io(process.env.REACT_APP_API_URL || 'https://192.168.7.20:3000', {
        auth: { token }
      });
